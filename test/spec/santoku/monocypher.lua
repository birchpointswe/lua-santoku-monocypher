...
test("unwrap_key with wrong wrap fails", function ()
  local id = crypto.derive_identity("test-secret")
  local key = crypto.derive_key("test-secret", id)
  local wrap1 = str.rep(str.char(0x42), 32)
  local wrap2 = str.rep(str.char(0x43), 32)
...
test("hmac_sha1 rfc 2202 vectors", function ()
  ...
  assert(crypto.hmac_sha1(str.rep(bytes(0x0b), 20), "Hi There")
    == "b617318655057264e28bc0b6fb378c8ef146be00")
  assert(crypto.hmac_sha1("Jefe", "what do ya want for nothing?")
    == "effcdf6ae5eb2fa2d27416d5f184df9c259a7c79")
  assert(crypto.hmac_sha1(str.rep(bytes(0xaa), 20), str.rep(bytes(0xdd), 50))
    == "125d7342b9ac11cd91a39af48aa17b4f63f175d3")
  local key4 = {}
  for i = 1, 25 do key4[i] = i end
  assert(crypto.hmac_sha1(bytes(arr.spread(key4)), str.rep(bytes(0xcd), 50))
    == "4c9007f4026250c6bc8414f9bf50c86c2d7235da")
  assert(crypto.hmac_sha1(str.rep(bytes(0x0c), 20), "Test With Truncation")
    == "4c1a03424b55e07fe7f27be1d58bb9324a9a5a04")
  assert(crypto.hmac_sha1(str.rep(bytes(0xaa), 80),
...
test("phrase_audit flags weak patterns and passes random phrases", function ()
  assert(crypto.phrase_audit("dog dog canyon ethics sushi hazelnut") == "repeated_word")
  assert(crypto.phrase_audit("sand sushi salad sixth sizzle serving") == "same_letter")
  assert(crypto.phrase_audit("apple canyon ethics hazelnut opera trend") == "alphabetical")
  assert(crypto.phrase_audit("trend opera hazelnut ethics canyon apple") == "alphabetical")
  assert(crypto.phrase_audit("apple bat crab dove echo flag") == "sequential")
  assert(crypto.phrase_audit("canyon ethics sushi hazelnut opera trend") == nil)
  assert(crypto.phrase_audit("") == nil)
end)

test("wipe is idempotent and blocks further use", function ()
  local id = crypto.derive_identity("test-secret", 1024, 1)
  local key = crypto.derive_key("test-secret", id)
  assert(key:wiped() == false)
  assert(id:wiped() == false)
  key:wipe()
  key:wipe()
  assert(key:wiped() == true)
  assert(not pcall(function () return key:bytes() end))
  assert(not pcall(function () return key:export() end))
  assert(not pcall(function () return key:derive("db") end))
  assert(not pcall(function () return key:encrypt("hello") end))
  id:wipe()
  id:wipe()
  assert(id:wiped() == true)
  assert(not pcall(function () return id:sub() end))
  assert(not pcall(function () return id:export() end))
...
local function unhex (h)
  return (h:gsub("%x%x", function (b) return str.char(tonumber(b, 16)) end))
end
...
test("p256 rejects a tampered signature, digest or key", function ()
  ...
  local sig = crypto.p256_sign(priv, digest)
  local bad_sig = str.char((sig:byte(1) + 1) % 256) .. sig:sub(2)
...
test("p256 verifies a known NIST P-256 vector", function ()
  ...
  assert(crypto.p256_verify(pub, sig, digest) == true)
  local bad = str.char((digest:byte(1) + 1) % 256) .. digest:sub(2)
